...
static HPyContext* _g_ctx;
    ...
    const static auto comp = [](HPy const & lhs, HPy const & rhs){
    ...
    using namespace std;
    const static auto comp = [](HPy const & lhs, HPy const & rhs){
        return (bool)HPy_RichCompareBool(lhs, rhs, HPy_LT);
    };
    using pb_set = set<HPy, decltype(comp)>;
#endif
...
struct Set4PyLong {
    ...
    Set4PyLong(vector<HPy> vec) : st(vec.begin(), vec.end(), comp), it(st.begin()) {}
    // maybe it needs dup:
    // Set4PyLong(const Set4PyLong& other) : st(other.st), it(st.begin()) {}

    bool add(HPy x) {
        HPy add_x = dup(x);
        const auto& r = st.insert(add_x);
    ...
    HPy to_list() {
        ...
        int i = 0;
        for (HPy p : st) {
            HPyListBuilder_Set(_g_ctx, builder, i++, dup(p));  // dup?
        }
    ...
    long index(HPy x) {
            ...
            while (it2 != st.end() && comp(*it2, x)) it2++, res++;
            return res;
        #endif
    }
};
